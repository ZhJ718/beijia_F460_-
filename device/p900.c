...
/****************************************************************
@FUNCTION：P900底层初始化
@INPUT：硬件接口函数结构体，P900底层管理结构体指针
@OUTPUT：无
@RETURN: 无
@AUTHOR：xfw
@SPECIAL: 绑定底层硬件操作函数，初始化，拉低电源使能引脚
****************************************************************/
extern void P900_LowLevelInit(P900_HW_OPERATIONS a_sHwOps, P900_LOWLEVEL_MAG *pMag)
{
    assert_param(pMag != NULL);

    memset(pMag, 0, sizeof(P900_LOWLEVEL_MAG));
...
@SPECIAL:
****************************************************************/
void P900_PowerOn(P900_LOWLEVEL_MAG *pMag)
{
...
@SPECIAL:
****************************************************************/
void P900_PowerOff(P900_LOWLEVEL_MAG *pMag)
{
    if(pMag->m_sHwOps.gpio_power_off != NULL)
    {
        pMag->m_sHwOps.gpio_power_off();
