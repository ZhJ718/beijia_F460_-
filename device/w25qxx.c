...
u8 W25QXX_ReadSR(W25QXX_HW_OPERATIONS a_sHwOps)
{
    ...
    a_sHwOps.reset_cs();	 //使能器件
    a_sHwOps.rwbyte(W25X_ReadStatusReg); //发送读取状态寄存器命令
    byte = a_sHwOps.rwbyte(0Xff);        //读取一个字节
    a_sHwOps.set_cs();	   //取消片选
    return byte;
}
//写W25QXX状态寄存器
...
{
    a_sHwOps.reset_cs();	 //使能器件
    a_sHwOps.rwbyte(W25X_WriteStatusReg);//发送写取状态寄存器命令
    a_sHwOps.rwbyte(sr);               	//写入一个字节
    a_sHwOps.set_cs();	   //取消片选
}
//W25QXX写使能
//将WEL置位
...
{
    a_sHwOps.reset_cs();	 //使能器件
    a_sHwOps.rwbyte(W25X_WriteEnable); 	//发送写使能
    a_sHwOps.set_cs();	   //取消片选
}
//W25QXX写禁止
//将WEL清零
...
{
    a_sHwOps.reset_cs();	 //使能器件
    a_sHwOps.rwbyte(W25X_WriteDisable);  //发送写禁止指令
    a_sHwOps.set_cs();	   //取消片选
}
//读取芯片ID
//返回值如下:
...
void W25QXX_Read(W25QXX_HW_OPERATIONS a_sHwOps, u8* pBuffer, u32 ReadAddr, u16 NumByteToRead)
{
    ...
    a_sHwOps.reset_cs();	 //使能器件
    a_sHwOps.rwbyte(W25X_ReadData);         	//发送读取命令
    a_sHwOps.rwbyte((u8)((ReadAddr) >> 16));  	//发送24bit地址
    a_sHwOps.rwbyte((u8)((ReadAddr) >> 8));
    a_sHwOps.rwbyte((u8)ReadAddr);

    for(i = 0; i < NumByteToRead; i++)
    {
        pBuffer[i] = a_sHwOps.rwbyte(0XFF);   	//循环读数
    }
...
void W25QXX_Write_Page(W25QXX_HW_OPERATIONS a_sHwOps, u8* pBuffer, u32 WriteAddr, u16 NumByteToWrite)
{
    ...
    a_sHwOps.reset_cs();	 //使能器件
    a_sHwOps.rwbyte(W25X_PageProgram);      	//发送写页命令
    a_sHwOps.rwbyte((u8)((WriteAddr) >> 16)); 	//发送24bit地址
    a_sHwOps.rwbyte((u8)((WriteAddr) >> 8));
    a_sHwOps.rwbyte((u8)WriteAddr);

    for(i = 0; i < NumByteToWrite; i++)
        a_sHwOps.rwbyte(pBuffer[i]);//循环写数

    a_sHwOps.set_cs();	   //取消片选
    W25QXX_Wait_Busy(a_sHwOps);					   		//等待写入结束
}
//无检验写SPI FLASH
...
void W25QXX_Erase_Chip(W25QXX_HW_OPERATIONS a_sHwOps)
{
    ...
    a_sHwOps.reset_cs();	 //使能器件
    a_sHwOps.rwbyte(W25X_ChipErase);        	//发送片擦除命令
    a_sHwOps.set_cs();	   //取消片选
    W25QXX_Wait_Busy(a_sHwOps);   				   		//等待芯片擦除结束
}
...
void W25QXX_Erase_Sector(W25QXX_HW_OPERATIONS a_sHwOps, u32 Dst_Addr)
{
    ...
    W25QXX_Write_Enable(a_sHwOps);                  	//SET WEL
    W25QXX_Wait_Busy(a_sHwOps);
    a_sHwOps.reset_cs();	 //使能器件   a_sHwOps.reset_cs();	 //使能器件
    a_sHwOps.rwbyte(W25X_SectorErase);      	//发送扇区擦除指令
    a_sHwOps.rwbyte((u8)((Dst_Addr) >> 16));  	//发送24bit地址
    a_sHwOps.rwbyte((u8)((Dst_Addr) >> 8));
    a_sHwOps.rwbyte((u8)Dst_Addr);
    a_sHwOps.set_cs();	   //取消片选
    W25QXX_Wait_Busy(a_sHwOps);   				   		//等待擦除完成
}
...
void W25QXX_Erase_Page(W25QXX_HW_OPERATIONS a_sHwOps, u32 Dst_Addr)
{
    ...
    W25QXX_Wait_Busy(a_sHwOps);
    a_sHwOps.reset_cs();	 //使能器件   a_sHwOps.reset_cs();	 //使能器件
    a_sHwOps.rwbyte(W25X_PageErase);      	//发送扇区擦除指令
    a_sHwOps.rwbyte((u8)((Dst_Addr) >> 16));  	//发送24bit地址
    a_sHwOps.rwbyte((u8)((Dst_Addr) >> 8));
    a_sHwOps.rwbyte((u8)Dst_Addr);
    a_sHwOps.set_cs();	   //取消片选
...
{
    while((W25QXX_ReadSR(a_sHwOps) & 0x01) == 0x01);  		// 等待BUSY位清空
}
//等待空闲
u32 W25QXX_Is_Busy(W25QXX_HW_OPERATIONS a_sHwOps)
{
    return W25QXX_ReadSR(a_sHwOps) & 0x01;  		// 等待BUSY位清空
}
...
{
    a_sHwOps.reset_cs();	 //使能器件
    a_sHwOps.rwbyte(W25X_PowerDown);        //发送掉电命令
    a_sHwOps.set_cs();	   //取消片选
    //delay_us(3);                               //等待TPD
}
//唤醒
